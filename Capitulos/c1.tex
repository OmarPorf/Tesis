...
\begin{proof}
	...
	\[
		\inner*{x} =\prod_{k < n}\pa*{f\pa*{k + 1}}^{\PROYECCION{n}{k + 1}\pa*{x}}
	\]
...
\begin{proof}
	...
	\[
		\varphi\pa*{n} =\pa*{n > 0\land\pa*{\forall p\leq n}\pa*{\pa*{P\pa*{p}\land D\pa*{p, x}}\to\pa*{\forall q\leq p}\pa*{P\pa*{q}\to D\pa*{q, x}}}}
	\]
...
\begin{theorem}
	Las funciones máxima potencia, acceso y longitud satisfacen que son primitivo-recursivas.
\end{theorem}
\begin{proof}
...
\begin{proof}
	...
	\[
		\operatorname{Max}\pa*{e} =\CARACTERISTICA{\SECUENCIAS}\pa*{e}\sum_{m = 0}^{e}k\CARACTERISTICA{M}\pa*{e, m}
	\]
...
La primer lista $\pa*{3, 2, 3, 5}$ se codificaría como $\inner*{3, 2, 3, 5} = 2^{4}3^{3}5^{4}7^{6}$ por ejemplo. Las demás listas se codificarían de la siguiente forma:
\[
	\begin{NiceArray}{c}
...
En este caso se obtiene ahora que $e\bra*{1} =\inner*{1, 1}$ es el código de la primer instrucción $S\pa*{1}$, $e\bra*{2} =\inner*{1, 3}$ el de la segunda instrucción $S\pa*{3}$, $e\bra*{3} =\inner*{3, 2, 3, 5}$ el de la tercera instrucción $J\pa*{2, 3, 5}$ y $e\bra*{4} =\inner*{3, 1, 1, 1}$ el de la cuarta instrucción $J\pa*{1, 1, 1}$.
...
El objetivo ahora es poder codificar la ejecución de una máquina URM de manera primitvo-recursiva. Para ello, primero debe codificarse el contenido de los registros en una iteración de la máquina. Para ello, recordemos que las ejecuciones de la máquina son una sucesión de funciones $\set*{f_{k}}_{k\in\omega}$ que representan el estado de la cinta en cada iteración $k\in\omega$ (sin considerar los valores $f_{k}(0)$). Por ejemplo, considerando de nuevo el ejemplo de la máquina $M = \pa*{J\pa*{2, 3, 5}, S\pa*{1}, S\pa*{3}, J\pa*{1, 1, 1}}$, tenemos que la sucesión de funciones que representan el estado de la cinta es la siguiente:
\[
	\begin{NiceArray}{cc}
...
Luego, podemos considerar únicamente listas de longitud mayor a $\operatorname{MaxReg}\pa*{e}$ para codificar cada función $f_{k}$. La longitud se considera mayor a $\operatorname{MaxReg}\pa*{e}$ para tener acceso a esos registros que necesita la máquina para ser ejecutada, además de considerar el caso cuando la entrada $f_{0}$ considere una lista más larga que $\operatorname{MaxReg}\pa*{e}$. En nuestro ejemplo, si $e = \inner*{0, \inner*{3, 2, 3, 5},\inner*{1, 1},\inner*{1, 3},\inner*{3, 1, 1, 1}}$, entonces tenemos que $\operatorname{MaxReg}\pa*{e} = 3$. Como la entrada consta de solo dos valores, podemos considerar una lista de longitud $5 >\operatorname{MaxReg}\pa*{e}$. Entonces cada una de las funciones $f_{k}$ se codificaría de la siguiente manera:
\[
...
\begin{definition}[Cómputo]
	...
	\begin{enumerate}
		...
		\item dada la ejecución $\set*{f_{k}}_{k\in\omega}$ de la máquina codificada por $e$, existe $k\in\omega$ tal que $c$ codifica a $f_{k}$ y $d$ codifica a $f_{k + 1}$
	\end{enumerate}
\end{definition}
...
En segundo lugar, la primer configuración corresponde a ingresar los datos de entrada a la cinta; la lista $z$. Para que tengamos espacio suficiente para registrar todos los datos de $z$ en la cinta inicial, la longitud de la cinta debe ser al menos la longitud de $z$ (más uno para guardar la instrucción a ejecutar). Por lo tanto, debe satisfacerse que $\LONGITUD\pa*{p\bra*{0}} >\LONGITUD\pa*{z}$, para cada $k <\LONGITUD\pa*{z}$ se cumple $p\bra*{0}\bra*{k + 1} = z\bra*{k}$ y para cada $k <\LONGITUD\pa*{p\bra*{0}}$, si $k >\LONGITUD\pa*{z}$, entonces $p\bra*{0}\bra*{k} = 0$. Además, como la primer instrucción a ejecutar siempre debe ser la primera, se debe cumplir también que $p\bra*{0}\bra*{0} = 1$.
...
\begin{definition}[T-predicado de Kleene]
	...
	\begin{enumerate}
		...
		\item para cada $k >\LONGITUD\pa*{z}$ se satisface $p\bra*{0}\bra*{k} = 0$,
...
\begin{proof}
	La demostración de esto se sigue directamente por todo lo que hemos construido. Si $\PHI{M}[n]\pa*{x}\ACABA$, entonces sabemos que existe una ejecución $p\in\omega$ para la cual $T\pa*{e, z, p}$, y luego $\mu p T\pa*{e, z, p}\ACABA$. Entonces por definición de la función $U$ obtenemos que $\PHI{M}[n]\pa*{x} = U\pa*{\mu p T\pa*{e, z, p}}$. 
	
	Si por el contrario se tiene que $\PHI{M}[n]\pa*{x}\DIVERGE$, entonces no existe ninguna ejecución $p\in\omega$ para la cual $T\pa*{e, z, p}$, y luego $\mu p T\pa*{e, z, p}\DIVERGE$. Por lo que $\PHI{M}[n]\pa*{x} = U\pa*{\mu p T\pa*{e, z, p}}\DIVERGE$.
\end{proof}
...
\begin{proof}
	...
	\]
	Puesto que el T-predicado de Kleene y la función $U$ son primitivo-primitivo recursivas, entonces $f$ es parcial-recursiva.
\end{proof}
...
\begin{notation}
	Sea $N\in\URM$ la máquina universal, entonces denotaremos por $\Phi_{e}\pa*{z}$ a $\PHI{N}[2](e, z)$ para cada $e, z\in\omega$. 
\end{notation}
